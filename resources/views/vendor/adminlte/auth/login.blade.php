...
@section('auth_body')
    <form action="{{ $login_url }}" method="post">
        ...
        {{-- Password field --}}
        <div class="input-group mb-3">
            <input type="password" id="input-pwd" name="password" class="form-control @error('password') is-invalid @enderror"
                   placeholder="{{ __('adminlte::adminlte.password') }}">
                <button type="button" id="icon-show-pwd" class="btn btn-default btn-xs fas fa-eye-slash"> Show</button>&nbsp;&nbsp;
            <div class="input-group-append">
                <div class="input-group-text">
    ...
    <script>
        function showPassword(icon_component, input_component, input_component_text) {
            icon_component.click(function() {
                ...
                icon_component.text() == " Show" ? icon_component.text(" Hide") : icon_component.text(" Show");
        ...
        $(document).ready(function () {
                var icon = $('#icon-show-pwd');
                var input = $('#input-pwd');
                showPassword(icon, input, "Show Password");
        });
